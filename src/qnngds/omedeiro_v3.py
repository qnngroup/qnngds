...
def ntron_single(
    ...
    for i in [(-0, 150), (150, 0), (0, -150), (-150, 0)]:
        ntron = qg.ntron_sharp(
            choke_w, choke_l, gate_w, channel_w, source_w, drain_w, layer
        )
        ntron.rotate(j)
        n1 = D << ntron
    ...
    field = pg.compass_multi(
        size=(field_x, field_x), ports=conn_dict, flip_ports=True, layer=10
    )
    ...
    for i in range(len(route_list)):
        ...
        t = D << qg.hyper_taper(15, f_ports[0].width * 0.75, routing)
...
def ntron_ind3(
    ...
    for i in range(len(route_list)):
        t = D << pg.outline(
            qg.hyper_taper(30, f_ports[0].width * 0.75, routing),
            distance=outline_dis,
            open_ports=True,
        )
        t.connect(t.ports[2], f_ports[route_list[i]].rotate(180))
        r = D << qg.outline(
            pr.route_manhattan(port1=port_list[i], port2=t.ports[1], radius=15),
            distance=outline_dis,
            open_ports=0,
            rotate_ports=True,
        )
...
def ntron_ind3_gnd(
    ...
    for i in range(len(route_list)):
        ...
        t.connect(t.ports[2], f_ports[route_list[i]])
        r = D << qg.outline(
            pr.route_manhattan(port1=port_list[i], port2=t.ports[1], radius=5),
            distance=outline_dis,
            open_ports=0,
            rotate_ports=True,
        )
...
def ntron_10g(
    ...
    conn_dict = {"N": 3, "W": 3, "S": 3, "E": 3}
    enc = qg.ntron_multi_gate_dual_fanout(
        num_gate,
        gate_w,
        gate_p,
        choke_w,
...
def ntron_10g_ind(
    ...
    conn_dict = {"N": 3, "W": 3, "S": 3, "E": 3}
    enc = qg.ntron_multi_gate_dual_fanout_ind(
        num_gate,
        gate_w,
        gate_p,
        choke_w,
...
def ntron_snspd(
    det_width=0.1, inductor_width=0.3, choke_w=0.02, channel_w=0.12, layer=1
):
    info = locals()
    ...
    det = pg.snspd(wire_width=det_width, wire_pitch=0.3, size=(3, 3))
    det.rotate(90)

    print(det.info)
    d = D << det

    s1 = D << pg.optimal_step(
        det_width, inductor_width, symmetric=True, anticrowding_factor=4
    )
    s1.connect(s1.ports[1], d.ports[2])
    s2 = D << pg.optimal_step(
        det_width, inductor_width, symmetric=True, anticrowding_factor=4
    )
    s2.connect(s2.ports[1], d.ports[1])

    tee1 = D << pg.tee(
        size=(1, inductor_width), stub_size=(inductor_width, 1), taper_type="fillet"
    )
    ...
    print(L1.info)
    ntron = D << qg.ntron_sharp(
        choke_w=choke_w,
        channel_w=channel_w,
        gate_w=inductor_width,
        layer=0,
    )
    ntron.connect(ntron.ports["g"], L1.ports[2])

    L2 = D << pg.snspd(
        wire_width=inductor_width, wire_pitch=inductor_width * 2, size=(36, 18)
    )
    L2.mirror()
    print(L2.info)
    ...
    L3 = D << pg.snspd(
        wire_width=inductor_width, wire_pitch=inductor_width * 3, size=(36, 18)
    )
    ...
    routs1 = D << pg.optimal_step(
        inductor_width, routing, symmetric=True, anticrowding_factor=4
    )
    routs1.connect(routs1.ports[1], L2.ports[2])
    routs2 = D << pg.optimal_step(
        inductor_width, routing, symmetric=True, anticrowding_factor=4
    )
...
def ntron_andor(choke_w=0.02, channel_w=0.12, inductor_width=0.3, layer=1):
    D = Device("ntron_andor")
    info = locals()
    routing = 3

    ntron1 = D << qg.ntron_sharp(
        choke_w=choke_w, channel_w=channel_w, gate_w=inductor_width, layer=0
    )
    ntron2 = D << qg.ntron_sharp(
        choke_w=choke_w, channel_w=channel_w, gate_w=inductor_width, layer=0
    )
    ntron2.mirror()
    ntron2.movex(15)
    ...
    L1 = D << pg.snspd(
        wire_width=inductor_width, wire_pitch=inductor_width * 2, size=(9, 5)
    )
    print(L1.info)
    ...
    L3 = D << qg.snspd_vert(
        wire_width=inductor_width, wire_pitch=inductor_width * 2, size=(15, 15)
    )
    print(L3.info)
    L3.connect(L3.ports[1], tee1.ports[3])

    routs1 = D << pg.optimal_step(
        inductor_width, routing, symmetric=True, anticrowding_factor=1.5
    )
...
def ntron_not(
    ...
    ntron1 = D << qg.ntron_sharp(
        ...
        channel_w=channel_w,
        gate_w=gate_w,
        drain_w=inductor_width,
        source_w=inductor_width,
        layer=0,
    )

    tee1 = D << pg.tee(
        size=(3, inductor_width), stub_size=(inductor_width, 1), taper_type="fillet"
    )
    tee1.connect(tee1.ports[1], ntron1.ports["s"])

    step1 = D << pg.optimal_step(
        inductor_width, nw_w, symmetric=True, anticrowding_factor=1.5
    )
    step1.connect(step1.ports[1], tee1.ports[2])

    L1 = D << pg.snspd(wire_width=nw_w, wire_pitch=0.2, size=nw_area)
    print(L1.info)
    L1.connect(L1.ports[1], step1.ports[2])

    st1 = D << pg.straight(size=(0.1, 0.5))
    st1.connect(st1.ports[1], L1.ports[2])

    ht1 = D << qg.hyper_taper(1, 5, 0.1)
    ht1.connect(ht1.ports[1], st1.ports[2])

    L2 = D << pg.snspd(
        wire_width=inductor_width,
        wire_pitch=inductor_width * 2,
        size=(inductor_width * 30, inductor_width * 20),
    )
    print(L2.info)
    L2.connect(L2.ports[1], ntron1.ports["d"])

    tee2 = D << pg.tee(
        size=(5, inductor_width), stub_size=(inductor_width, 1), taper_type="fillet"
    )
    ...
    routs4 = D << pg.optimal_step(
        gate_w, routing, symmetric=True, anticrowding_factor=1.5
    )
...
def straight_wire_pad_bilayer(
    ...
    ######################################################################
    detector = qg.outline(
        pg.straight((width, length)), distance=outline, open_ports=2, layer=layer
    )

    pad = qg.pad_U(pad_width=pad_width, layer=pad_layer)
    pad.rotate(180)

    pad_taper = qg.outline(
        qg.hyper_taper(40, pad_width + 4 * pad_outline, width * step_scale),
        distance=outline,
        open_ports=2,
        layer=layer,
    )
    ...
    ground_taper = qg.outline(
        qg.hyper_taper(
            ground_taper_length, pad_width + 4 * pad_outline, width * step_scale
        ),
        distance=outline,
        open_ports=2,
        layer=layer,
    ...
    if de_etch:
        pad_etch = D << pg.rectangle(
            size=(pad_width - 40, pad_width - 40), layer=de_etch
        )
...
def snspd_straight_ind_symdiff(
    width=1, length=30, a1=300, a2=100, inductor_width_factor=2, layer=1
):
    ...
    inductor1 = pg.snspd(
        wire_width=width * iwf, wire_pitch=width * iwf * 3, size=(a1, a2)
    )
    L1 = D << inductor1
    ...
    D = pg.union(D)
    D.info = info
    D.info["inductor_squares"] = (
        inductor1.info["num_squares"] + inductor2.info["num_squares"]
    )
...
def snspd_straight_ind_symdiff2(
    width=1, length=30, a1=300, a2=100, inductor_width_factor=3, layer=1
):
...
def snspd_straight_ind_symdiff3(
    width=1, length=30, a1=300, a2=100, inductor_width_factor=3, layer=1
):
    ...
    D.info["total_squares"] = (
        length / width + inductor1.info["num_squares"] + inductor2.info["num_squares"]
    )
    D.add_port(name=1, port=g1.ports[2])
    D.add_port(name=2, port=g2.ports[2])
...
def single_cell(device_layer=1, heater_layer=2, loop_adjust=0):
    ...
    det = D << qg.snspd_vert(
        wire_width=0.1, wire_pitch=0.6, size=(10, 10), layer=device_layer
    )

    taper = pg.optimal_step(
        0.1, 2, symmetric=True, anticrowding_factor=1, layer=device_layer
    )
    ...
    rs1 = D << pr.route_smooth(
        tee1.ports[3], tee2.ports[3], path_type="U", length1=10, layer=device_layer
    )
    port1 = tee1.ports[1]
    port2 = tee2.ports[2]
...
def tap_array(n, width=1, tap_spacing=100, pad_extension=10, alternate=False, layer=1):
    ...
    tee = pg.tee(
        size=(tap_spacing, width),
        stub_size=(width * 4, pad_extension),
        taper_type="fillet",
    )
    t_list = []
    port_list = []
...
def via_array(
    ...
    for i in range(0, n):
        conn = pg.straight(
            size=(via_size + via_inset * 2, spacing - via_size - 2 * via_inset),
            layer=layers[1 + (-1) ** (i + 1)],
        )
    ...
    cstart = D << pg.straight(
        size=(via_size + via_inset * 2, spacing - via_size - 2 * via_inset),
        layer=layers[2],
    )
    cstart.connect(cstart.ports[1], via_list[0].ports[2])

    pstart = D << qg.pad_basic(
        base_size=pad_size,
        port_size=via_size + via_inset * 2,
        taper_length=60,
        layer=layers[2],
    )
    pstart.connect(pstart.ports[1], cstart.ports[2])

    pstart_etch = D << pg.rectangle(
        size=(pad_size[0] + offset, pad_size[1] + offset), layer=layers[0]
    )
    ...
    for via, i in zip(via_list, range(1, len(via_list))):
        ...
        pint_etch = D << pg.rectangle(
            size=(pad_size[0] + offset, pad_size[1] + offset),
            layer=layers[1 + (-1) ** (i + 1)],
        )
        pint_etch.move(pint_etch.center, pint.ports["center"])

        if p == 4:
            pint_etch1 = D << pg.rectangle(
                size=(pad_size[0] - offset, pad_size[1] - offset), layer=layers[1]
            )
    ...
    if n % 2 == 0:
        pend_etch = D << pg.rectangle(
            size=(pad_size[0] + offset, pad_size[1] + offset), layer=layers[0]
        )
        pend_etch.move(pend_etch.center, pend.ports["center"])
    else:
        pend_etch = D << pg.rectangle(
            size=(pad_size[0] + offset, pad_size[1] + offset), layer=layers[2]
        )
        pend_etch.move(pend_etch.center, pend.ports["center"])
        pend_etch1 = D << pg.rectangle(
            size=(pad_size[0] - offset, pad_size[1] - offset), layer=layers[1]
        )
...
def htron_alt(
    heater_size=(0.2, 4), channel_size=(1, 4), route_width=4, h_layer=1, c_layer=2
):
    D = Device("htron")
    ...
    channel_taper = D << pg.optimal_step(
        channel_size[0],
        route_width,
        anticrowding_factor=0.2,
        symmetric="True",
        layer=c_layer,
    )
    channel_taper.connect(channel_taper.ports[1], channel.ports[1])
    channel_taper1 = D << pg.optimal_step(
        channel_size[0],
        route_width,
        anticrowding_factor=0.2,
        symmetric="True",
    ...
    flagpole1 = D << pg.flagpole(
        size=(route_width * 2, route_width),
        stub_size=(heater_size[0], channel_size[1]),
        shape="p",
        taper_type="fillet",
        layer=h_layer,
    )
    flagpole2 = D << pg.flagpole(
        size=(route_width * 2, route_width),
        stub_size=(heater_size[0], channel_size[1]),
        shape="d",
        taper_type="fillet",
    ...
    port_list = [
        channel_taper.ports[2],
        channel_taper1.ports[2],
        compass2.ports["E"],
        compass1.ports["E"],
    ]
...
def htron_not(
    ...
    heater1 = D << pg.snspd(
        hwidth1, hwidth1 * 2, size=size, terminals_same_side=True, layer=hl
    )
    heater1.move(heater1.center, wire1.center)
    ...
    tee1 = D << pg.tee(
        size=(route_width * 2, width1),
        stub_size=(route_width, width1 * 2),
        taper_type="fillet",
        layer=dl,
    )
    tee2 = D << pg.tee(
    ...
    step1 = D << pg.optimal_step(
        width1, width2, anticrowding_factor=1.5, num_pts=100, symmetric=True, layer=dl
    )
    step1.connect(step1.ports[1], tee1.ports[2])
    ...
    step2 = D << pg.optimal_step(
        route_width,
        width2,
        anticrowding_factor=1.5,
        num_pts=100,
        symmetric=True,
        layer=dl,
    )
    ...
    tee3 = D << pg.tee(
        size=(route_width * 5, width1),
        stub_size=(width1, width1 * 10),
        taper_type="fillet",
        layer=dl,
    )
    ...
    induct1 = D << pg.snspd(
        wire_width=width1, wire_pitch=width1 * 2, size=(20, 10), layer=dl
    )
    induct1.connect(induct1.ports[1], tee3.ports[3])

    D.info["induct_info"] = induct1.info

    step3 = D << pg.optimal_step(
        width1, route_width, anticrowding_factor=1.5, symmetric=True, layer=dl
    )
    step3.connect(step3.ports[1], tee3.ports[1])

    step4 = D << pg.optimal_step(
        width1, route_width, anticrowding_factor=1.5, symmetric=True, layer=dl
    )
...
def htron_notR(
    ...
    heater1 = D << pg.snspd(
        hwidth1, hwidth1 * 2, size=size, terminals_same_side=True, layer=dl2
    )
    heater1.move(heater1.center, wire1.center)
    ...
    step3 = D << pg.optimal_step(
        width1, route_width, anticrowding_factor=1.0, symmetric=True, layer=dl1
    )
    ...
    tee = pg.tee(
        size=(route_width * 2, route_width),
        stub_size=(route_width, route_width * 2),
        taper_type="fillet",
        layer=dl1,
    )
...
def htron_andor_snspd(
    width1=0.5, width2=0.1, length1=4, length2=4, hwidth1=0.15, hwidth2=0.5, dl=3, hl=1
):
    ...
    induct1 = D << pg.snspd(
        wire_width=width1, wire_pitch=width1 * 2, size=(12, 10), layer=dl
    )
    ...
    tee2 = D << pg.tee(
        size=(length1 * 3, width1),
        stub_size=(width1, width1 * 4),
        taper_type="fillet",
        layer=dl,
    )
    tee2.connect(tee2.ports[2], tee1.ports[3])

    snspd1 = D << pg.snspd_expanded(
        wire_width=width2,
        wire_pitch=width2 * 3,
        size=(10, 10),
        connector_width=width1,
        layer=dl,
    )
    snspd1.mirror()
    ...
    wire3 = D << pg.straight(
        size=(width1, snspd1.ports[2].midpoint[1] - wire2.ports[2].midpoint[1]),
        layer=dl,
    )
...
def meander_smooth(
    ...
    nlength = (meander_length - 2 * n * arc_length - 2 * r) / (
        n - 1
    )  # meander straight length
...
def die_edge(size0=300, size1=(7500, 7500), layer=0):
    ...
    D << qg.alignment_marks(
        locations=((-4000, -4000), (4000, 4000), (-4000, 4000), (4000, -4000))
    )
...
def die_cell(
    ...
    markerOpen.move(
        markerOpen.center,
        (D.bbox[0][0] + ground_width / 2, D.bbox[1][1] - ground_width / 2),
    )
    markerOpen = E << pg.rectangle(size=(150, 150))
    markerOpen.move(
        markerOpen.center,
        (D.bbox[1][0] - ground_width / 2, D.bbox[0][0] + ground_width / 2),
    )
...
def die_cell_v2(
    ...
    for p, i in zip(padloc_list, range(0, len(fieldloc_ports))):
        ...
        E << pr.route_sharp(
            pad.ports[1], pad2.ports[2], path_type="Z", length1=30, length2=10
        )
