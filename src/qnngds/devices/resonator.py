...
def transmission_line_resonator(tl_width, tl_length, meander_width, meander_e_eff, res_freq, turn_radius, 
                                meander_gap=1, area_length=100, layer=0, layer_metal=1, cpw=False, 
                                bounding_box=False, box_buffer=50, box_layer=200, overlap=0,
                                layer_nb=2, meandered=True, final_cpw=True):
    '''
    Builds a resonator in the middle of two transmission lines, returned as pg.Device
    ...
    tl_device = transmission_line(tl_width, meander_width, tl_length, gap=meander_gap, layer=layer)
    if meandered:
        res_device, res_area_height = resonator(meander_e_eff, res_freq, meander_width, turn_radius, area_length, layer=layer, cpw=cpw, gap=meander_gap)

        total_length = tl_length*2 + res_area_height
        print('Total height of meander: '+str(total_length))
    else:
        res_device = straight_resonator(meander_e_eff, res_freq, meander_width, layer=layer, cpw=cpw, gap=meander_gap)
    ...
    pad_ref = device << pads(contact_width=tl_width, layer_sc=layer, layer_metal=layer_metal)
    pad_ref.connect(pad_ref.ports['connect'], device.ports['bottom'])

    if final_cpw:
        sc_polys = pg.extract(device, layers=[layer])
        sc_polys.add_port('gnd', midpoint=(0, sc_polys.ymin), width=tl_width, orientation=-90)
    ...
    if bounding_box:
        ...
        tl_box = pg.bbox(np.add(device.bbox, [[-box_buffer/2, 5],[box_buffer/2, box_buffer/2]]), layer=box_layer)
        device << tl_box
...
def optimal_90deg_as_taper(narrow_width, port_offset=0, layer=0):
    ...
    device.add_port('wide', midpoint=(0, port_offset), width=(device.xmax-device.xmin), orientation=-90)
    device.add_port('narrow', midpoint=(0, device.ymax), width=narrow_width, orientation=90)
...
def ic_device(width, length, gap=0, layer=0, layer_metal=1, cpw=False, bounding_box=False, 
              box_buffer=50, box_layer=200, shield=False, overlap=None, layer_nb=2, pad_size=150,
              layer_cover=300, final_cpw=True, via_layer=105, layer_eb=109):
    '''
    ...
    taper = optimal_90deg_as_taper(width*(1+optimal_width), port_offset=width*(1+optimal_width)/2, layer=layer)
    taper_height = (taper.ymax - taper.ymin) + (taper_init.xmax - taper_init.xmin)

    print(f'10% taper length: {taper_init.xmax - taper_init.xmin:0.1f} \n wrt width: {(taper_init.xmax - taper_init.xmin)/width:0.1f}')
    ...
    pad_ref = device << pads(contact_width=150, contact_length=150, cpw=cpw, gap=gap, layer_sc=layer, layer_metal=layer_metal)
    pad_ref.connect(pad_ref.ports['connect'], ref_t1.ports['wide'])

    if shield:
        cover = pg.rectangle((width-2*overlap, length+2*taper_height), layer=301)
        ...
        ebeam_nb = pg.rectangle((6*width, length+2*taper_height - width), layer=layer_eb)
    ...
    if final_cpw:
        sc_polys = pg.extract(device, layers=[layer])
        sc_polys.add_port('gnd', midpoint=(0, sc_polys.ymin), width=sc_polys.xmax - sc_polys.xmin, orientation=-90)
        ...
        other_polys = pg.extract(device, layers=[layer_metal, layer_nb, box_layer, via_layer, layer_eb])
        device = pg.Device()
        device << sc
...
def tl_only(width, length, layer=0, layer_metal=1, cpw=False, gap=0, bounding_box=False, box_buffer=50, box_layer=200):
    line = pg.rectangle((length, width), layer=layer)
    line.center = [0, 0]
    ...
    #tl_d.rotate(0)
    pad_ref = device << pads(contact_width=width, cpw=cpw, gap=gap, layer_sc = layer, layer_metal=layer_metal)
...
def pads(contact_width=300, extra_space=0.1, layer_sc=0, layer_metal=1, cpw=False, gap=0, 
         contact_length=300):
    '''
    ...
    pad << pg.compass(size=(contact_width-extra_space, contact_length-extra_space), layer=layer_metal)

    return pad

def pinhole_test(size=400, x_overlap=100, layer_sc=0, layer_metal=1, cpw=False, gap=0, 
                 bounding_box=True, box_layer=200, box_buffer=50):
    test = pg.Device()
    ...
    if bounding_box:
        tl_box = pg.bbox(np.add(test.bbox, [[-box_buffer/2, -box_buffer/2],[box_buffer/2, box_buffer/2]]), layer=box_layer)
        test << tl_box

    return test

def transmission_line(width1, width2, length, layer=0, cpw=False, gap=0, nw_gap=0, taper_gap=0):
    '''
    Builds a transmission line smoothly tapered to narrower contact, returned as pg.Device
...
def resonator(e_eff, res_freq, width, turn_radius, area_length, layer=0, cpw=False, gap=0):
    '''
    Builds a resonator at a given resonant frequency, returned as pg.Device (meandering)
...
def straight_resonator(e_eff, res_freq, width, layer=0, cpw=False, gap=0):
    '''
    Builds a straight resonator at a given resonant frequency, returned as pg.Device
    (not meandered)
    ...
    device.add_port('bottom', midpoint=(-res_length/2, 0), width=width, orientation=180)
    if not cpw:
        return device
    else:
...
def compute_res_wavelength(e_eff, res_freq):
    '''
    Computes resonant wavelength from effective permittivity, resonant frequency
    '''
    v = compute_veff(e_eff)
    return v/res_freq*1e6

def meander(total_length, width, turn_radius, area_length, layer=0, cpw=False, gap=0, tolerance=1):
    '''
    defines a uniform-width meander of a given total length that fits into a given
    area width
    ...
    '''
    gap = 0
    odd_turn = pg.arc(width=width, radius=turn_radius, theta=180, start_angle=-90, layer=layer, angle_resolution=0.5)
    even_turn = pg.arc(width=width, radius=turn_radius, theta=180, start_angle=90, layer=layer, angle_resolution=0.5)
    first_attach = pg.arc(width=width, radius=turn_radius, theta=-90, start_angle=-90, layer=layer, angle_resolution=0.5)
    last_attach_odd = pg.arc(width=width, radius=turn_radius, theta=-90, start_angle=90, layer=layer, angle_resolution=0.5)
    last_attach_even = pg.arc(width=width, radius=turn_radius, theta=90, start_angle=90, layer=layer, angle_resolution=0.5)
    '''if cpw:
    ...
    resulting_length = (num_lines+1)*(area_length+np.pi*turn_radius)+np.pi*turn_radius
    ...
    half_line.add_port('left', midpoint=(-area_length/4, 0), orientation=180, width=width)
    half_line.add_port('right', midpoint=(area_length/4, 0), orientation=0, width=width)
...
def ground_with_cutouts(width, height, device, cuts_layer=200, layer=0, text_layer=107):
    '''
    Returns full ground cover pattern with cutouts for devices, as pg.Device
    ...
    ground = pg.kl_boolean(ground, pg.extract(device, layers=[cuts_layer, text_layer]), 'A-B', layer=layer)
