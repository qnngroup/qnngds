...
def packer(
    ...
    """Returns Device "p" with references from D_list. Names, or index, of each
    device is assigned and can be called from p.references[i].parent.name.
    ...
    for i in range(len(p[0].references)):
        ...
        text_object = pg.text(
            text=device_text, size=text_height, justify="left", layer=text_layer
        )
...
def packer_rect(
    device_list, dimensions, spacing, text_pos=None, text_size=50, text_layer=1
):
    """This function distributes devices from a list onto a rectangular grid.
    The aspect ratio (dimensions) and spacing must be specified. If specified,
    text can be added automatically in a A1, B2, C3, style. The text will start
    with A0 in the NW corner.
    ...
    for i in range(dimensions[0]):
        for j in range(dimensions[1]):
            if not new_shape[i][j] == None:
                moved_device = new_shape[i][j].move(
                    origin=new_shape[i][j].bbox[0],
                    destination=(i * spacing[0], -j * spacing[1]),
                )
                ...
                if text_pos:
                    ...
                    text_object.move(
                        destination=(
                            i * spacing[0] + text_pos[0],
                            -j * spacing[1] + text_pos[1],
                        )
                    )
...
def packer_doc(D_pack_list):
    """This function creates a text document to be referenced during
    meansurement. Its primary purpose is to serve as a reference for device
    specifications on chip. For instance, "A2 is a 3um device.".
...
def text_labels(device_list, adjust_position=(0, 0), text_size=40, layer=1):
    """SORTA BROKEN This function accepts list of device objects. Each
    coordinate is extracted from the list and the text is generated
    automatically.
    ...
    for i in range(len(x_reduced)):
        for j in range(len(y_reduced)):
            ...
            text_object = pg.text(
                text=device_text, size=text_size, justify="left", layer=layer
            )
            text_object.move(
                destination=(
                    x_reduced[i] + adjust_position[0],
                    y_reduced[j] + adjust_position[1],
                )
            )
