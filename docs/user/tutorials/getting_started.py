...
ALIGN_CELL_LEFT = design.create_alignment_cell(
    layers_to_align=[layers["mgb2_coarse"], layers["pad"]], text="LEFT"
)
design.place_on_chip(ALIGN_CELL_LEFT, (0, 2))

ALIGN_CELL_RIGHT = design.create_alignment_cell(
    layers_to_align=[layers["mgb2_coarse"], layers["pad"]], text="RIGHT"
)
design.place_on_chip(ALIGN_CELL_RIGHT, (4, 2))

VDP_TEST_MGB2 = design.create_vdp_cell(
...
VDP_TEST_PAD = design.create_vdp_cell(
    layers_to_probe=[layers["mgb2_coarse"], layers["pad"]],
    layers_to_outline=[layers["mgb2_coarse"]],
    text="PAD & MGB2",
)
design.place_on_chip(VDP_TEST_PAD, (0, 1))

RES_TEST_MGB2_FINE = design.create_resolution_test_cell(
    layer_to_resolve=layers["mgb2_fine"], text="MGB2 FINE"
)
design.place_on_chip(RES_TEST_MGB2_FINE, (2, 2))

RES_TEST_MGB2_COARSE = design.create_resolution_test_cell(
    layer_to_resolve=layers["mgb2_coarse"], text="MGB2 COARSE"
)
design.place_on_chip(RES_TEST_MGB2_COARSE, (1, 2))

RES_TEST_PAD = design.create_resolution_test_cell(
    layer_to_resolve=layers["pad"],
    resolutions_to_test=[0.5, 0.75, 1, 1.25, 1.5, 1.75, 2.0],
    text="PAD",
)
design.place_on_chip(RES_TEST_PAD, (3, 2))

ETCH_TEST = design.create_etch_test_cell(layers_to_etch=[[layers["pad"]]], text="PAD")
...
NANOWIRES = design.create_nanowires_cell(
